...
import OrderDetailsDialog from '../order-details-dialog/index'; // Импортируем компонент модалки

// Определяем интерфейс для данных заказа
interface OrderData {
  id: number;
  order_number: string;
  client_name: string;
...
const fetchWashing = async (search: string, page: string, size: string) => {
  const res = await $api.get('/master/order-groups/?status=completed', {
    params: {
      search: search,
      page: page,
      size: size,
...
const DetailingCompletedTable = () => {
  ...
  // Состояния для модального окна с деталями
  ...
  // Функция для получения детальных данных о заказе и открытия модального окна
  const handleRowClick = async (order: OrderData) => {
    try {
      setIsLoadingDetails(true);

      // Получаем детальные данные заказа из API
      ...
      // Если не удалось получить детали, используем данные из списка
...
export default DetailingCompletedTable;
