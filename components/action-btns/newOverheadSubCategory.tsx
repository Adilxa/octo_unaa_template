...
const NewOverheadSubCategory = ({ category }: { category: string }) => {
  const queryClient: any = useQueryClient();
  const [categoryName, setCategoryName] = useState('');

  const createNewCategory = async () => {
    try {
      await $api.post('overhead/subcategories/', {
        name: categoryName,
        category,
      });
      await queryClient.invalidateQueries('overheadSubCategoriesList');
...
export default NewOverheadSubCategory;
