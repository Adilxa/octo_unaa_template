...
const TabSwitcher: React.FC<Props> = ({ tabArr, paramName }) => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const currentTab: any = searchParams.get(paramName ? paramName : 'period');
  ...
  const onSetQueryParam = (param: string, additionalParams: Record<string, string> = {}) => {
    ...
    params.set(paramName ? paramName : 'period', param);
