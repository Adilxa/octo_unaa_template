...
const fetchData = async (period: string): Promise<ExpenseDataItem[]> => {
  const res = await $api.get<ExpenseDataItem[]>('expenses/expense-sums/', {
...
const ExpenseChart: React.FC = () => {
  ...
  const period = searchParams.get('expensePeriod') || 'day';

  const { data: apiData, isLoading } = useQuery({
    queryKey: ['ExpenseChart', period],
    ...
    <div className='h-full w-full rounded-xl bg-[#171928] p-6 shadow'>
      <div className='mb-6 flex items-center justify-between'>
        <div className={'flex items-center gap-10'}>
          <div className='text-[20px] text-white'>Расход в период {getPeriodText()}</div>
          <div className='text-[32px] font-semibold text-[#0A63F0]'>
            -{totalIncome.toLocaleString('ru-RU').replace(',', ' ')} сом
          </div>
        </div>
        <TabSwitcher paramName={'expensePeriod'} tabArr={tabsArr} />
      </div>
